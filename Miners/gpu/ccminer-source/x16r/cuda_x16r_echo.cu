...
__device__ static uint32_t c_Message80[20];
...
void x16r_echo512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint64_t *g_hash, uint32_t *g_nonceVector)
{
...
__host__
void x16r_echo512_cpu_init(int thr_id, uint32_t threads)
{
	aes_cpu_init(thr_id);
}

__host__
void x16r_echo512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order)
{
	...
	x16r_echo512_gpu_hash_64<<<grid, block>>>(threads, startNounce, (uint64_t*)d_hash, d_nonceVector);
	MyStreamSynchronize(NULL, order, thr_id);
}

__host__
void x16r_echo512_cpu_setBlock_80(void *pdata)
{
	cudaMemcpyToSymbol(c_Message80, pdata, 80, 0, cudaMemcpyHostToDevice);
}

__device__ __forceinline__
void cuda_echo_round_80(
	const uint32_t *const __restrict__ sharedMemory, uint32_t nonce, uint32_t *const __restrict__  hash)
{
    uint32_t k0 = 640;
    uint32_t W[64];
	...
	for (int i = 0; i < 16; i++)
	{
		hash[i] = c_Message80[i] ^ W[i];
    }
}

__global__ __launch_bounds__(128, 7) /* will force 72 registers */
void x16r_echo512_gpu_hash_80(uint32_t threads, uint32_t startNonce, uint64_t *g_hash)
{
	__shared__ uint32_t sharedMemory[1024];
	...
	if (thread < threads)
	{
		...
		int hashPosition = nonce - startNonce;
		uint32_t *Hash = (uint32_t*)&g_hash[hashPosition<<3];

		cuda_echo_round_80(sharedMemory, nonce, Hash);
	}
}

__host__
void x16r_echo512_cpu_hash_80(int thr_id, uint32_t threads, uint32_t startNonce, uint32_t *d_hash, int order)
{
	const uint32_t threadsperblock = 128;
	...
	x16r_echo512_gpu_hash_80<<<grid, block>>>(threads, startNonce, (uint64_t*) d_hash);
	MyStreamSynchronize(NULL, order, thr_id);
}